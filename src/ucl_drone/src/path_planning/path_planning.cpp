...
bool PathPlanning::xy_desired()
{
  ...
  if (sqrt((lastPoseReceived.x - next_x) * (lastPoseReceived.x - next_x) +
           (lastPoseReceived.y - next_y) * (lastPoseReceived.y - next_y)) < 0.05)
  {
    printf("i: %d\n", i);
    // if (i % 2 == 0)
    // {
    ...
    double labyrinth_x[] = {0, 2.5, 2.5, 0, 0};
    double labyrinth_y[] = {0, 0, 1.8, 1.8, 0};
    if (next_x != labyrinth_x[i])
    {
      double dir = sgn(labyrinth_x[i] - next_x);
      next_x += 0.15 * dir;
      if (dir * sgn(labyrinth_x[i] - next_x) == -1)
        next_x = labyrinth_x[i];
    }
    if (next_y != labyrinth_y[i])
    {
      double dir = sgn(labyrinth_y[i] - next_y);
      next_y += 0.15 * dir;
      if (dir * sgn(labyrinth_y[i] - next_y) == -1)
        next_y = labyrinth_y[i];
    }
    if (next_x == labyrinth_x[i] && next_y == labyrinth_y[i])
    {
      i++;
    }
