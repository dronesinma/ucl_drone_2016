...
BasicController::BasicController()
{
  ...
  /*
  List of subscribers and publishers. This node subscribes to pose_estimation to get the real-time
  position of the drone. It also subsribes to path_planning to know the new pose that the drone must
  reach.

  In order to do that, this node publish in the topic cmd_vel in order to give the computed speed
  commands to the drone. It also can send message in order to make the drone taking off or landing
  and to change its state (emergency or ok).

  The message reset pose is used to reset the pose when the drone finishes to take off. It allows
  the pose_estimation to reset and to erase the drift that happenned during launching.
  ...
  */
  ...
  startControl_ =
      nh.advertiseService("drone_ucl/start_control", &BasicController::startControl, this);

  stopControl_ = nh.advertiseService("drone_ucl/stop_control", &BasicController::stopControl, this);
  ...
  /*Here are the gains used for the PID controller. Those are tuned for an AR.Drone 2.0 with the
   * indoor skin.
   */
...
bool BasicController::startControl(std_srvs::Empty::Request& req, std_srvs::Empty::Response& res)
{
  ...
  sendVelToDrone(0, 0, 0, 0,
                 true);  // very important: ensure the state at next start is hover mode (see
                         // ardrone_autonomy doc)
  isControlling = true;
  ros::Duration(8).sleep();  // Wait for 8s before to reset the odometry (in order to erase
                             // drift).
  std_msgs::Empty msg;
  reset_pub.publish(msg);
  ros::Duration(4).sleep();  // Wait for 4s
  return true;
}

bool BasicController::stopControl(std_srvs::Empty::Request& req, std_srvs::Empty::Response& res)
{
  // ROS_INFO("calling service stop");
  sendVelToDrone(0, 0, 0, 0);  // very important: ensure the state at next start is hover mode (see
                               // ardrone_autonomy doc)
  isControlling = false;
...
void BasicController::reguXY(double* xvel_cmd, double* yvel_cmd, double x_mes, double y_mes,
...
void BasicController::reguAltitude(double* zvel_cmd, double alt_mes, double alt_desired,
                                   double regu_new_time_z)
{
  ...
  if (time_difference != 0)
  {
    if (alt_desired_old != alt_desired)  // Reset of the integral term if target has changed
    {
      integral_alt_error = 0;
    }
...
void BasicController::reguYaw(double* yawvel_cmd, double yaw_mes, double yaw_desired,
                              double regu_new_time)
{
  ...
  if (time_difference != 0)
  {
    if (yaw_desired_old != yaw_desired)  // Reset of yaw integral error if the target has changed.
    {
      integral_yaw_error = 0;
    }
    ...
    // The yaw angle is included in [-pi;pi] interval. In order to make it go from -179° to 179°
    // without taking the large side (passing through 0) we implemented a way to make it take the
    // shorter way to go to a position. If its proportional term is greater than a half turn, we
...
void BasicController::sendVelToDrone(double pitch, double roll, double yaw_vel, double zvel_cmd,
                                     bool force /*=false*/)
{
...
// This function is really important. It is here that all the regulation functions are called and
